...
@section('content')
<div class="container">
    <h3 class="mb-4">Editar Campanha</h3>
    ...
    <form action="{{ route('campanha.atualizar', $campanha->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <input type="date" name="data" class="form-control" value="{{ old('data', $campanha->data) }}" required>
        </div>

        <div class="mb-3">
            <label for="status" class="form-label">Status</label>
            <select name="status" class="form-select" required>
                <option value="1" {{ old('status', $campanha->status) == '1' ? 'selected' : '' }}>Ativa</option>
                <option value="0" {{ old('status', $campanha->status) == '0' ? 'selected' : '' }}>Inativa</option>
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="id_grupo_cidades" class="form-select" required>
                ...
                <option value="{{ $grupo->id }}" {{ old('id_grupo_cidades', $campanha->id_grupo_cidades) == $grupo->id ? 'selected' : '' }}>
                    {{ $grupo->nome_grupo }}
                </option>
                @endforeach
            </select>
            ...
            <div class="form-check">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="produtos[]" value="{{ $produto->id }}"
                        {{ (is_array(old('produtos')) && in_array($produto->id, old('produtos'))) || (!old('produtos') && $campanha->produtos->contains($produto->id)) ? 'checked' : '' }}>
